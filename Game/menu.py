...
class Menu:
    ...
    def afficheInGame(self, screen, playerEnJeu):
        for player in playerEnJeu:
            player.paquet.affiche(screen)
            pygame.draw.rect(screen, 'red', player.paquet, 2)
            screen.blit(self.police[2].render(player.nom, True, 'black'), (player.paquet.rect.x, player.paquet.rect.x - 10))


    def lancer(self):
        return self.start_button_rect.collidepoint(pygame.mouse.get_pos()) and pygame.mouse.get_pressed()[0] and all(len(name) > 1 for name in self.names)
